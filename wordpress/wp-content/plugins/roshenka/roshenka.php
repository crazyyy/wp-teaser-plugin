...
add_action('init', array( 'roshenka_fixed_widget', 'init' )); // Main Hook

if ( class_exists('roshenka_fixed_widget', false) ) return; // if class is allready loaded return control to the main script

class roshenka_fixed_widget { // Plugin class

  const ID = 'roshenka_fixed_widget';

  const VERSION = '2016.09.04';
  ...
  public static function enqueue_scripts() {
    ...
    wp_enqueue_script(self::ID, plugin_dir_url( __FILE__ ) . 'js/roshenka-front.js', array('jquery'), self::VERSION, true);

  }

  public static function is_widget_fixed($instance, $widget, $args){

    if ( isset($instance['roshenka_fixed_widget']) && $instance['roshenka_fixed_widget'] ) {
  ...
  public static function get_widget_sidebar($widget_id) {
    ...
    return 'roshenka-default-sidebar';
  ...
  public static function js_settings() {
    ...
    if ( is_array(self::$fixed_widgets) && !empty(self::$fixed_widgets) ) {
      ...
      $js .= 'var roshenka_sidebar_options = new Array();'.PHP_EOL;
      ...
      foreach ( self::$fixed_widgets as $sidebar => $widgets ) {
        ...
        $js .= 'roshenka_sidebar_options['. $i .'] = { "sidebar" : "'. $sidebar .'", "margin_top" : '. $options['margin-top'] .', "margin_bottom" : '. $options['margin-bottom'] .', "stop_id" : "' . $options['stop-id'] .'", "screen_max_width" : '. $options['screen-max-width'] .', "screen_max_height" : '. $options['screen-max-height'] .', "width_inherit" : '. $width_inherit .', "refresh_interval" : '. $refresh_interval .', "window_load_hook" : '. $window_load_hook .', "disable_mo_api" : '. $disable_mo_api .', "widgets" : ['. $widgets_array .'] };'.PHP_EOL;
  ...
  public static function add_widget_option($widget, $return, $instance) {
    ...
    if ( isset($instance['roshenka_fixed_widget']) ) $iqfw = $instance['roshenka_fixed_widget']; else $iqfw = 0;
  ...
  public static function update_widget_option($instance, $new_instance, $old_instance){
      ...
      if ( isset($new_instance['roshenka_fixed_widget']) && $new_instance['roshenka_fixed_widget'] ) {

        $instance['roshenka_fixed_widget'] = 1;

      } else {

        $instance['roshenka_fixed_widget'] = false;
  ...
  protected static function load_language() {
    ...
    load_plugin_textdomain( 'roshenka-fixed-widget', false, $languages_path );

  }

  public static function admin_menu() {

    remove_action('admin_menu', array( 'roshenka_fixed_widget', 'admin_menu' )); // Remove free version plugin

    self::$settings_page_hook = add_submenu_page( 'themes.php', __('Roshenka Options', 'roshenka-fixed-widget'), __('Roshenka Options', 'roshenka-fixed-widget'), 'activate_plugins', self::ID, array( __CLASS__, 'settings_page' ) );
  ...
  protected static function load_options() {
    ...
    $options_old = get_option('roshenka_fixed_widget');
  ...
  public static function settings_page() {
    ...
    // add_meta_box(self::ID.'-custom-ids', __('Custom IDs', 'roshenka-fixed-widget'), array( __CLASS__, 'settings_page_custom_ids_box' ), $screen, 'normal');

    $options = self::load_options();

    echo '<div class="wrap"><div id="icon-themes" class="icon32"><br /></div><h2>'. __('Fixed Widget Options', 'roshenka-fixed-widget') .'</h2>'.PHP_EOL;
    ...
    // echo '<p><em>'. __("Note for cache plugin users. Don't forget to clear the cache after options change!", 'roshenka-fixed-widget') .'</em></p>'.PHP_EOL;
  ...
  public static function settings_page_general_box($options) {
    ...
    echo '<p><span style="display: inline-block; width: 150px;">'. __('Margin Top:', 'roshenka-fixed-widget') .'</span><input type="text" name="'. self::ID .'[margin-top]" value="'. $options['margin-top'] .'" style="width: 50px; text-align: center;" />&nbsp;'. __('px', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 150px;">'. __('Margin Bottom:', 'roshenka-fixed-widget') .'</span><input type="text" name="'. self::ID .'[margin-bottom]" value="'. $options['margin-bottom'] .'" style="width: 50px; text-align: center;" />&nbsp;'. __('px', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    // echo '<p><span style="display: inline-block; width: 150px;">'. __('Stop ID:', 'roshenka-fixed-widget') .'</span><input type="text" name="'. self::ID .'[stop-id]" value="'. $options['stop-id'] .'" style="width: 150px;">&nbsp;'. __('Here you need to provide html tag id. The position of that html element will determine margin bottom value.', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    // echo '<p><span style="display: inline-block; width: 150px;">'. __('Refresh interval:', 'roshenka-fixed-widget') .'</span><input type="text" name="'. self::ID .'[refresh-interval]" value="'. $options['refresh-interval'] .'" style="width: 50px; text-align: center;" />&nbsp;'. __('milliseconds', 'roshenka-fixed-widget') .' / '. __('Used only for compatibility with browsers without MutationObserver API support! Set 0 to completely disable.', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 150px;">'. __('Disable Width:', 'roshenka-fixed-widget') .'</span><input type="text" name="'. self::ID .'[screen-max-width]" value="'. $options['screen-max-width'] .'" style="width: 50px; text-align: center;" />&nbsp;'. __('px', 'roshenka-fixed-widget') .' / '. __('use this option to disable plugin on portable devices. ') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 150px;">'. __('Disable Height:', 'roshenka-fixed-widget') .'</span><input type="text" name="'. self::ID .'[screen-max-height]" value="'. $options['screen-max-height'] .'" style="width: 50px; text-align: center;" />&nbsp;'. __('px', 'roshenka-fixed-widget') .' / '. __('use this option to disable plugin on portable devices.', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

  }

  public static function settings_page_custom_ids_box($options) {

    echo '<p><span >'. __('Custom HTML IDs (each one on a new line):', 'roshenka-fixed-widget') .'</span><br/><br/><textarea name="'. self::ID .'[custom-ids]" style="width: 320px; height: 120px;">'. $options['custom-ids'] .'</textarea>'.PHP_EOL;

  }

  public static function settings_page_compatibility_box($options) {

    echo '<p><span style="display: inline-block; width: 280px;">'. __('Auto fix widget id:', 'roshenka-fixed-widget') .'</span><input type="checkbox" name="'. self::ID .'[fix-widget-id]" value="yes" '. checked('yes', $options['fix-widget-id'], false) .' /> </p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 280px;">'. __('Disable MutationObserver:', 'roshenka-fixed-widget') .'</span><input type="checkbox" name="'. self::ID .'[disable-mo-api]" value="yes" '. checked('yes', $options['disable-mo-api'], false) .' /> '. __('If MutationObserver disabled, plugin will use Refresh Interval to reflect page changes (version 4 behavior)', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 280px;">'. __('Enable plugin for logged in users only:', 'roshenka-fixed-widget') .'</span><input type="checkbox" name="'. self::ID .'[logged_in_req]" value="yes" '. checked('yes', $options['logged_in_req'], false) .' /> '. __('Enable this option for debug purposes (frontend problems and etc.)', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 280px;">'. __('Inherit widget width from the parent container:', 'roshenka-fixed-widget') .'</span><input type="checkbox" name="'. self::ID .'[width-inherit]" value="yes" '. checked('yes', $options['width-inherit'], false) .' /> '. __('Enable this option for themes with responsive sidebar', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 280px;">'. __('Use jQuery(window).load() hook:', 'roshenka-fixed-widget') .'</span><input type="checkbox" name="'. self::ID .'[window-load-enabled]" value="yes" '. checked('yes', $options['window-load-enabled'], false) .' /> '. __('Enable this option only if you have problems with other scroll oriented javascript code', 'roshenka-fixed-widget') .'</p>'.PHP_EOL;

    echo '<p><span style="display: inline-block; width: 280px;">'. __('widget_display_callback hook priority:', 'roshenka-fixed-widget') .'</span><select name="'. self::ID .'[widget_display_callback_priority]"><option value="1" '. selected('1', $options['widget_display_callback_priority'], false) .'>1</option><option value="10" '. selected('10', $options['widget_display_callback_priority'], false) .'>10</option><option value="20" '. selected('20', $options['widget_display_callback_priority'], false) .'>20</option><option value="30" '. selected('30', $options['widget_display_callback_priority'], false) .'>30</option><option value="50" '. selected('50', $options['widget_display_callback_priority'], false) .'>50</option><option value="100" '. selected('100', $options['widget_display_callback_priority'], false) .'>100</option></select></p>'.PHP_EOL;
...
} // roshenka_fixed_widget_pro class
